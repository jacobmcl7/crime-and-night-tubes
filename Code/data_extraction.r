...
crime_data <- as.data.frame(read.csv("Data/2015-01/2015-01-metropolitan-street.csv", stringsAsFactors = FALSE))
...
for (i in 2:12) {
    ...
    # read in the data
    temp_data <- as.data.frame(read.csv(paste0("Data/2015-", month, "/2015-", month, "-metropolitan-street.csv"), stringsAsFactors = FALSE))
...
for (j in 2016:2017) {
    for (i in 1:12) {
        # get the correct month format
        month <- ifelse(i < 10, paste0("0", i), i)
        temp_data <- as.data.frame(read.csv(paste0("Data/", j, "-", month, "/", j, "-", month, "-metropolitan-street.csv"), stringsAsFactors = FALSE))
        crime_data <- rbind(crime_data, temp_data)
    }  
}
...
# for some reason this gives a corrupted file: also export it as csv
write.csv(crime_data, 'Data/london_crime_data.csv')